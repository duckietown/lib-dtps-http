"""Memory dashboard just push test."""
...
async def async_main() -> None:
    """Run asynchronously."""
    every_once_in_a_while = EveryOnceInAWhile(2)
    number_of_bytes = 0
    ...
    async with context_cleanup("switchboard", environment) as context:
        ...
        topic = context / "topic1"
        await topic.queue_create(bounds=bounds)
        number_received = 0
        received_bytes = 0
        on_data = get_on_data(number_received, received_bytes)
        await topic.subscribe(on_data, inline=True)
        async with topic.publisher_context() as publisher:
            while True:
                await asyncio.sleep(PUBLISH_PERIOD)
                if number_of_messages > number_received + 10:
                    logger.info("Moderating...")
                    continue
                ...
                if every_once_in_a_while.now():
                    ...
                    if len(delta_times) > MAXIMUM_DELTA_TIMES_LENGTH:
                        ...
                        logger.info("Average push delay: %.3fms", average_ms)
