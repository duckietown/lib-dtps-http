...
async fn check_exists(ss_mutex:
                      Arc<Mutex<ServerState>>, topic_name: String) -> Result<String, warp::Rejection>
...
async fn handle_connection_generic(ws: warp::ws::WebSocket, state: Arc<Mutex<ServerState>>,
    ...
    loop {
        ...
        match r {
            Ok(message) => {
                println!("Received update for topic {}: index {}", topic_name, message);
